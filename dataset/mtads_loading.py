...
def check_suite_availability(benchmark: str = "fsb") -> None:
    base_path = Path(get_default_path(benchmark))

    available = False
    if (base_path.exists()
            and len(list(base_path.glob("*.yaml"))) > 0
            and len(list(base_path.glob("*/*.csv"))) > 0):
        available = True
    if available:
        # all good
...
def load_all_m_tads_stored_datasets(benchmark: str = "fsb") -> Generator:
    ...
    def _load_from_path(path):
        if path.name in dataset_cache:
            ...
            if train_sequence.exists():
                train_sequence = _drop_not_relevant_columns(
                    pd.read_csv(train_sequence.absolute())
                )
            else:
            ...
            if test_sequence.exists():
                test_sequence = _drop_not_relevant_columns(
                    pd.read_csv(test_sequence.absolute())
                )
            else:
                raise FileExistsError("Test sequence not available")

            dataset_cache[path.name] = {
                "cache": 5,
...
def load_all_stored_datasets(args: dataclass) -> Generator:
    if type(args) is str:
        _args = make_dataclass('RunArguments', [('dataset', str)])(**{'dataset': args})
    else:
        _args = args
    if "dataset" not in [f.name for f in fields(_args)]:
        raise ValueError(
            'dataset not specified, parameter must be a dataclass containing "dataset"'
        )
...
if __name__ == "__main__":
    ...
    sets = list(
        load_all_stored_datasets(
            make_dataclass("RunArguments", [("dataset", str)])(**{"dataset": "real"})
        )
    )
    for i, (n, c, _, _) in enumerate(sets):
        if n == "smd_machine-1-1":
            print(i, n, c)
